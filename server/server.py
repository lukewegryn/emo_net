...
#model.load('./model/checkpoint_emo_net-9000') # This one worked really well! Maybe we can do better though...
model.load('./model/checkpoint_emo_net-45200')
...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if r:
            ...
            f.write(b"{status:'failed',error:'Could not find a face in the image...'}")
        #f.write(info.encode())
        length = f.tell()
        f.seek(0)
    ...
    def deal_post_data(self):
        ...
        line = self.rfile.readline()
        remainbytes -= len(line)
        fn = re.findall(r'Content-Disposition.*name="webcam"; filename="(.*)"', line.decode())
        if not fn:
        ...
        while remainbytes > 0:
            ...
            if boundary in line:
                ...
                out.close()
                #own code here
                result = analyze_file(fn,model)[0]
                if result == False:
                    return(False,"")
                prediction = emotion_dict[np.argmax(result)]
                dict_string = '["Angry", "Disgust", "Fear", "Happy", "Sad", "Surprise", "Neutral"]'
                return (True, '"status":"success","emotion":"{0}","result":{1},"dict_string":{2}'.format(prediction, result, dict_string))
            else:
                out.write(preline)
                preline = line
